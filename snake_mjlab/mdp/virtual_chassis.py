"""Virtual chassis computation — pure PyTorch, framework-agnostic."""

import torch


def quat_from_matrix(m: torch.Tensor) -> torch.Tensor:
    """Convert batched orthonormal 3x3 rotation matrices to quaternions in (w, x, y, z)."""
    batch = m.shape[0]
    trace = m[:, 0, 0] + m[:, 1, 1] + m[:, 2, 2]
    q = torch.zeros(batch, 4, device=m.device, dtype=m.dtype)

    # w = sqrt(1 + trace) / 2
    q[:, 0] = torch.sqrt(1.0 + trace) / 2.0

    # x, y, z computed from off-diagonal elements
    q[:, 1] = (m[:, 2, 1] - m[:, 1, 2]).clamp(min=0.0).sqrt()
    q[:, 2] = (m[:, 0, 2] - m[:, 2, 0]).clamp(min=0.0).sqrt()
    q[:, 3] = (m[:, 1, 0] - m[:, 0, 1]).clamp(min=0.0).sqrt()
...
def compute_virtual_chassis_frame(
    ...
    """Return the geometric center and principal axes of the virtual chassis via SVD."""
